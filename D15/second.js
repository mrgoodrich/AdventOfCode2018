...
var elfAttackPower = 15; // brute force min to still increase by 1 (also could have just let it count to 20)

while (!battle()) {
   elfAttackPower++;
   console.log('trying attack power ' + elfAttackPower);
}
...
//scan and insert elves and goblins, set empty where removed
   ...
   var originalElfCount = elves.length;

   // printGrid();

// while elves exist and goblins exist
   var round = 0;
   while (goblins.length && elves.length === originalElfCount) {
      round++;
      ...
      while (turnOrder.length) {
         ...
         if (player != null && didFind) {
            ...
            if (!goblins.length) {
               ...
               console.log('Minimum elf attack: ' + elfAttackPower);
               console.log("SECOND ANSWER: " + (round - 1) * endingHealth);
            ...
            function attack() {
               ...
               var damage = GOBLIN_ATTACK_POWER;
               if (toAttack.type === 'goblin') {
                  damage = elfAttackPower;
               }
               orig.hp -= damage;
               if (orig.hp <= 0) {
   ...
   console.log('returning ' + (elves.length === originalElfCount));
   return elves.length === originalElfCount;
