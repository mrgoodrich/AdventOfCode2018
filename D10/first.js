...
function move() {
   data.forEach((val,ndx) => { // array. value, index
      ...
      data[ndx].position[1] += val.velocity[1];

   });
}
var turns = 0;
function verifyNoLine() {
   turns++;
   var res = true;
   data.forEach((value,ndx) => { // array. value, index
      var thismatch = false
   	data.forEach((val2,ndx2) => { // array. value, index
         if (ndx != ndx2) {

            // console.log(value.position + ' and ' + val2.position);
            if (
               (
                  Math.abs(val2.position[0] - value.position[0]) > 1
                  // val2.position[0] !== value.position[0] &&
                  // val2.position[0] !== value.position[0]  - 1 &&
                  // val2.position[0] !== value.position[0] + 1
               )
               ||
               (
                  Math.abs(val2.position[1] - value.position[1]) > 1
                  // val2.position[1] !== value.position[1] &&
                  // val2.position[1] !== value.position[1] - 1 &&
                  // val2.position[1] !== value.position[1] + 1
               )
               ...
               // if (res) {
               //    console.log(value.position + ' and ' + val2.position);
               // console.log(val2.position[1])
               // console.log(value.position[1])
               // console.log(val2.position[0])
               // console.log(value.position[0])
               // }
               //nomatch
            } else {
               //match
